...
import { OrderItem } from '@domain/value-objects/order-item.js'
  ...
  describe('successful scenarios', () => {
    ...
    it('should add multiple items to order', async () => {
      ...
      if (isOk(orderResult)) {
        ...
        const laptopItem = updatedOrder.items.find((item: OrderItem) => item.productSku.value === 'LAPTOP-001')
        expect(laptopItem?.quantity.value).toBe(1)
        expect(laptopItem?.unitPrice.amount).toBe(999.99)

        const mouseItem = updatedOrder.items.find((item: OrderItem) => item.productSku.value === 'MOUSE-001')
